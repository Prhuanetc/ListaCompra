...
export default function Home() {
  ...
  const getItemsFromDevice = async ()=> {
    try {
      const itemsMomory = await AsyncStorage.getItem('ListaCompra');
      if (itemsMomory !=mull) {
        setItems(JSON.parse(itemsMomory));
      }
          } catch (error) {
  ...
  const saveItemToDevice = async () => {
    try {
      ...
      await AsyncStorage.setItem('ListaCompra', ItemJson);
    } catch (error) {
      console.log(`Erro: ${error}`)
    }
  }
  ...
  const removeItem = itemId => {
    Alert.alert(
      'Excluir Produto?', 'Comfirmar a exclusão desse produto?',
      [
        {
          text: 'sim', onPress: () => {
  ...
  const removeAll = () => {
    Alert.alert(
      "limpar Lista?", "Comfirmar a exclusão de tdos os produtos?",
      [
        {
          text: 'Sim',
          onPress: () => { setItems([]) }
